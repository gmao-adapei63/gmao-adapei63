...
function persistActionsChange(){
    saveData();
    updateActionsBadge();
    if(document.getElementById('actions-overlay').classList.contains('active')) renderActionsList();
}
...
function toggleActionsSelectMode(){
    ...
    document.getElementById('actions-select-toggle').classList.toggle('active', actionsSelectMode);
    document.getElementById('actions-bulk-toolbar').style.display = actionsSelectMode ? 'flex' : 'none';
    renderActionsList();
}
...
function toggleActionSelected(id){
    ...
    document.getElementById('actions-bulk-count').textContent = actionsSelectedIds.size + ' sélectionnée(s)';
    renderActionsList();
}
...
function populateActionEditorForm(a){
    document.getElementById('action-modal-title').textContent = editingActionId ? ('Action ' + a.id) : 'Nouvelle Action';
    ...
    document.getElementById('action-pin-btn').classList.toggle('active', !!a.pinned);
    document.getElementById('action-delete-btn').style.display  = editingActionId ? '' : 'none';
    document.getElementById('action-archive-btn').style.display = editingActionId ? '' : 'none';
    document.getElementById('action-archive-btn').textContent = a.state === 'archived' ? '📤 Désarchiver' : '🗄️ Archiver';
...
function updateActionReportBox(){
    ...
    box.style.display = editingActionId ? 'flex' : 'none';
...
function toggleActionPin(){
    ...
    document.getElementById('action-pin-btn').classList.toggle('active', actionDraft.pinned);
}
...
function openPostponeDialog(){
    ...
    document.getElementById('action-postpone-old').textContent = 'Échéance actuelle : ' + oldDue;
    ...
    document.getElementById('actionPostponeModal').classList.add('active');
}
...
function openActionsTrash(){
    renderActionsTrashList();
    document.getElementById('actionsTrashModal').classList.add('active');
}
...
function printActionsHtml(list){
    ...
    document.getElementById('actions-print-area').innerHTML = html;
    window.print();
}
