...
function closeActionsModule(){
    ...
    const fab = document.getElementById('actions-fab');
    if(fab){
        if(typeof window.applyHomeVisibility === 'function') window.applyHomeVisibility();
...
function populateActionEditorForm(a){
    ...
    safeStyle('action-to-mission-btn','display', editingActionId ? '' : 'none');
...
// ─────────────────────────────────────────────────────────────────
// TRANSFORMER UNE ACTION (Note) EN MISSION — additif
// ─────────────────────────────────────────────────────────────────
// Crée une mission (appState.calendar[date]) à partir du titre/description/
// priorité/échéance de l'Action, puis place l'Action d'origine dans la
// corbeille (jamais de suppression définitive — récupérable comme
// n'importe quelle Action supprimée). Ne nécessite aucune modification
// du moteur des missions existant : on pousse simplement un objet au
// même format que celui déjà créé par saveTask() dans app.js.
function transformerActionEnMission(){
    if(!actionDraft || !editingActionId){
        showAlarmToast('⚠️','Action non enregistrée','Enregistrez d\'abord cette Action avant de la transformer en mission.','warning',4000);
        return;
    }
    readActionEditorFormIntoDraft();
    if(!actionDraft.title){
        showAlarmToast('⚠️','Titre requis','Merci de donner un titre à cette Action avant de la transformer.','warning',4000);
        return;
    }
    const dateVal = actionDraft.dueDate || new Date().toISOString().split('T')[0];
    if(!confirm('Transformer "' + actionDraft.title + '" en mission le ' + dateVal + ' ?\n\nL\'Action d\'origine sera placée dans la corbeille (récupérable à tout moment, rien n\'est perdu).')) return;

    if(!appState.calendar[dateVal]) appState.calendar[dateVal] = [];
    const priorityMap = {critique:'1', haute:'1', normale:'2', faible:'3'};
    const mission = {
        id: generateUUID(),
        title: actionDraft.title,
        desc: actionDraft.description || '',
        type: 'particular',
        priority: priorityMap[actionDraft.priority] || '2',
        timeStart: actionDraft.dueTime || '',
        timeEnd: '',
        assignedTo: [],
        equipment: [],
        interCompany: '',
        interName: '',
        comment: 'Créée depuis l\'Action ' + actionDraft.id,
        service: '',
        recRuleId: null,
        order: appState.calendar[dateVal].length,
        completed: false
    };
    appState.calendar[dateVal].push(mission);

    // Persiste les éventuelles modifications de dernière minute puis
    // archive l'Action d'origine dans la corbeille avec une trace claire
    // du lien vers la mission créée (historique, jamais silencieux).
    const idx = (appState.actions||[]).findIndex(a=>a.id===editingActionId);
    if(idx > -1){
        pushActionHistory(actionDraft, 'convert', 'Transformée en mission du ' + dateVal + ' (' + mission.id + ')');
        appState.actions[idx] = actionDraft;
        moveActionToTrash(editingActionId, true);
    }
    ...
    saveData();
    closeActionEditor();
    showAlarmToast('🎯','Mission créée', '"' + mission.title + '" ajoutée au ' + dateVal + '. L\'Action d\'origine est dans la corbeille.', 'success', 5000);

    // Si l'app affiche déjà des missions, on suit la nouvelle mission créée.
    const dateInput = document.getElementById('system-date');
    if(dateInput){ dateInput.value = dateVal; if(typeof loadDayData === 'function') loadDayData(dateVal); }
