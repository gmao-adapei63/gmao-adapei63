...
function printProcedure(){
    ...
    const html = `
        <div class="pp-header">
            <div class="pp-logo">${escapeProcHtml((appState.settings&&appState.settings.siteName)||'GMAO')}</div>
            <div class="pp-title">
                <h1>${escapeProcHtml(p.title)}</h1>
                <div>${cat.icon} ${cat.label}${p.subCategory?' — '+escapeProcHtml(p.subCategory):''}</div>
            </div>
            <div class="pp-meta">
        ...
        ${p.warnings.length ? `<div class="pp-alert">${p.warnings.map(escapeProcHtml).join('<br>')}</div>` : ''}
        ${p.ppe.length ? `<h2>EPI nécessaires</h2>${procListHtml(p.ppe)}` : ''}
        ${p.prerequisites.length ? `<h2>Prérequis</h2>${procListHtml(p.prerequisites)}` : ''}
        ${p.tools.length ? `<h2>Outils</h2>${procListHtml(p.tools)}` : ''}
        ${p.consumables.length ? `<h2>Consommables</h2>${procListHtml(p.consumables)}` : ''}
        <h2>Étapes</h2>
...
// ═════════════════════════════════════════════════════════════════
let importedAttachmentBuffer = null;
function openImportProcedureModal(){
    activeProtocolId = null;
    activeProtocolDraft = newProcedure();
    importedAttachmentBuffer = null;
...
function handleProcedureImportFile(file){
    if(!file) return;
    const textTypes = ['text/html','text/markdown','text/plain'];
    const ext = file.name.split('.').pop().toLowerCase();
    const isTextLike = textTypes.includes(file.type) || ['html','htm','md','markdown','txt'].includes(ext);
    ...
    if(isTextLike){
        ...
        // PDF / Word / image : conservé en pièce jointe, contenu à compléter manuellement
        const isImage = file.type.startsWith('image/');
        (isImage ? compressImageToDataURL(file,1600,0.75) : readFileAsDataURL(file)).then(dataUrl=>{
            importedAttachmentBuffer = {id:generateUUID(), name:file.name, type:file.type||'application/octet-stream', size:file.size, dataUrl, addedAt:new Date().toISOString()};
            showAlarmToast('📎','Fichier joint', `${file.name} sera ajouté en pièce jointe à la nouvelle procédure.`, 'success', 4000);
...
function analyzeProcedureImport(){
    ...
    if(importedAttachmentBuffer) activeProtocolDraft.attachments.push(importedAttachmentBuffer);
