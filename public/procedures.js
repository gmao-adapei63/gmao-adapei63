...
function newProcedure(){
    ...
    return {
        ...
        execUrl: '',                 // page HTML liée (ex. OneDrive) pour l'exécution terrain — bouton 🛂 persistant
        execFile: null,              // {name, html} — page HTML importée directement (contourne les blocages d'affichage OneDrive/SharePoint)
...
function normalizeProcedure(raw){
    ...
    if(typeof p.execUrl !== 'string') p.execUrl = '';
    if(!p.execFile || typeof p.execFile !== 'object' || !p.execFile.html) p.execFile = null;
...
function openCreateProtocolModal(){
    ...
    safeText('protocol-modal-title', 'Nouvelle Procédure');
    safeStyle('protocol-edit-btn','display', 'none');
    safeStyle('protocol-print-btn','display', 'none');
    safeStyle('protocol-view-panel','display', 'none');
    safeStyle('protocol-import-panel','display', 'none');
    safeStyle('protocol-link-panel','display', 'none');
    safeStyle('protocol-edit-panel','display', 'block');
    populateProtocolEditForm(activeProtocolDraft);
    renderProtocolModalFooter('edit');
    activateFirstTab('protocolModal');
    safeAddClass('protocolModal','active');
}
function openProtocolModal(id){
    ...
    safeText('protocol-modal-title', p.title || '(sans titre)');
    safeStyle('protocol-edit-btn','display', 'inline-flex');
    safeStyle('protocol-print-btn','display', 'inline-flex');
    safeStyle('protocol-import-panel','display', 'none');
    safeStyle('protocol-link-panel','display', 'none');
    safeStyle('protocol-edit-panel','display', 'none');
    safeStyle('protocol-view-panel','display', 'block');
...
function renderProtocolViewPanel(p){
    ...
    const hasExec = (p.execFile && p.execFile.html) || p.execUrl;
    const execBtn = hasExec ? `
        <button type="button" class="btn btn-primary" style="margin-top:10px;background:${cat.color}" onclick="execProcedureField('${p.id}')">
    ...
    safeHTML('protocol-view-panel', header
...
function populateProtocolEditForm(p){
    ...
    document.getElementById('proc-exec-url').value = p.execUrl || '';
    renderProcExecFileState();
...
function saveProtocol(){
    ...
    p.execUrl = document.getElementById('proc-exec-url').value.trim();
    // p.execFile est déjà à jour : addProcExecFile()/removeProcExecFile() le modifient
    // directement sur activeProtocolDraft (=== p), même principe que les pièces jointes.
...
// ── Bouton 🛂 persistant : ouvre la page terrain liée ──────────────
// Priorité au fichier HTML importé directement (execFile) : rendu via
// une URL blob locale, donc TOUJOURS affichable dans la GMAO, quel que
// soit le site d'origine. Le simple lien (execUrl) reste proposé mais
// certains sites (OneDrive/SharePoint notamment) refusent activement
// d'être encadrés dans une iframe pour des raisons de sécurité — un
// blocage décidé par Microsoft, que la GMAO ne peut pas contourner
// depuis une simple URL. Importer le fichier HTML résout ce problème.
function execProcedureField(id){
    const p = (appState.protocols||[]).find(x=>x.id===id) || activeProtocolDraft;
    if(!p) return;
    if(p.execFile && p.execFile.html){
        const blob = new Blob([p.execFile.html], {type:'text/html'});
        const blobUrl = URL.createObjectURL(blob);
        if(typeof window.openWebview === 'function') window.openWebview(blobUrl, p.title);
        else window.open(blobUrl, '_blank');
        return;
    }
    if(p.execUrl){
        if(typeof window.openExternalLink === 'function') window.openExternalLink(p.execUrl, p.title);
        else window.open(p.execUrl, '_blank');
        return;
    }
}
...
// ── Fichier HTML d'exécution terrain importé directement ──────────
// Contrairement à un simple lien, ce fichier est stocké tel quel (texte
// HTML) et rouvert via une URL blob locale au clic sur 🛂 : cela
// fonctionne TOUJOURS, y compris si le site d'origine (OneDrive,
// SharePoint...) refuse par sécurité d'être affiché dans une iframe.
function addProcExecFile(file){
    if(!file) return;
    const reader = new FileReader();
    reader.onload = () => {
        activeProtocolDraft.execFile = {name:file.name, html:reader.result};
        renderProcExecFileState();
        showAlarmToast('🛂','Page terrain importée', file.name, 'success', 3000);
    };
    reader.onerror = () => alert('Lecture du fichier impossible.');
    reader.readAsText(file, 'utf-8');
}
function removeProcExecFile(){
    activeProtocolDraft.execFile = null;
    document.getElementById('proc-exec-file').value = '';
    renderProcExecFileState();
}
function renderProcExecFileState(){
    const nameEl = document.getElementById('proc-exec-file-name');
    const removeBtn = document.getElementById('proc-exec-file-remove');
    if(!nameEl || !removeBtn) return;
    const f = activeProtocolDraft && activeProtocolDraft.execFile;
    nameEl.textContent = f ? `📄 ${f.name} (importé)` : '';
    removeBtn.style.display = f ? 'inline-flex' : 'none';
}
...
function printProcedure(){
    ...
    `;
    safeHTML('procedure-print-area', html);
    window.print();
}
...
function openImportProcedureModal(){
    ...
    safeText('protocol-modal-title', 'Importer une procédure');
    safeStyle('protocol-edit-btn','display', 'none');
    safeStyle('protocol-print-btn','display', 'none');
    safeStyle('protocol-view-panel','display', 'none');
    safeStyle('protocol-edit-panel','display', 'none');
    safeStyle('protocol-link-panel','display', 'none');
    safeStyle('protocol-import-panel','display', 'block');
    document.getElementById('proc-import-file').value = '';
    document.getElementById('proc-import-paste').value = '';
    safeHTML('protocol-modal-footer',
        `<button class="btn btn-secondary" onclick="closeModal('protocolModal')">Annuler</button>`);
    safeAddClass('protocolModal','active');
}

// ═════════════════════════════════════════════════════════════════
// 3e MODE DE CRÉATION — Lier une URL (ex. page HTML sur OneDrive)
// Brève analyse best-effort (le CORS bloque souvent la lecture d'un
// domaine tiers — dans ce cas on se contente du titre déduit de
// l'URL et l'utilisateur complète manuellement). L'adresse est de
// toute façon conservée comme lien d'exécution terrain (execUrl),
// accessible ensuite via le bouton 🛂 persistant sur la procédure.
// ═════════════════════════════════════════════════════════════════
function openLinkProcedureModal(){
    activeProtocolId = null;
    activeProtocolDraft = newProcedure();
    safeText('protocol-modal-title', 'Lier une page (URL)');
    safeStyle('protocol-edit-btn','display', 'none');
    safeStyle('protocol-print-btn','display', 'none');
    safeStyle('protocol-view-panel','display', 'none');
    safeStyle('protocol-edit-panel','display', 'none');
    safeStyle('protocol-import-panel','display', 'none');
    safeStyle('protocol-link-panel','display', 'block');
    document.getElementById('proc-link-url').value = '';
    document.getElementById('proc-link-status').textContent = '';
    safeHTML('protocol-modal-footer',
        `<button class="btn btn-secondary" onclick="closeModal('protocolModal')">Annuler</button>`);
    safeAddClass('protocolModal','active');
}
function analyzeProcedureLinkURL(){
    const url = document.getElementById('proc-link-url').value.trim();
    if(!url){ alert('Indiquez une adresse URL.'); return; }
    const statusEl = document.getElementById('proc-link-status');
    statusEl.textContent = '⏳ Analyse en cours…';

    const finalize = (parsed) => {
        activeProtocolDraft = normalizeProcedure(Object.assign(newProcedure(), parsed));
        activeProtocolDraft.execUrl = url;
        safeText('protocol-modal-title', 'Nouvelle Procédure (liée à une page)');
        safeStyle('protocol-link-panel','display', 'none');
        safeStyle('protocol-edit-panel','display', 'block');
        populateProtocolEditForm(activeProtocolDraft);
        renderProtocolModalFooter('edit');
    ...
    const fallbackTitle = hostname ? `Procédure liée — ${hostname}` : 'Procédure liée';
    ...
    fetch(url, {mode:'cors'}).then(r=>{
        ...
        let parsed;
        if(/<\/?[a-z][\s\S]*>/i.test(text)) parsed = parseHTMLToProcedure(text);
        else parsed = {title: fallbackTitle, description:'', steps:[]};
        if(!parsed.title) parsed.title = fallbackTitle;
        showAlarmToast('🔎','Page analysée', 'Champs pré-remplis à partir du contenu — à vérifier.', 'success', 4000);
        finalize(parsed);
    }).catch(()=>{
        // CORS ou page non accessible directement depuis le navigateur :
        // on continue quand même — l'URL reste liée, les champs restent à
        // compléter manuellement (catégorie, titre, etc. comme d'habitude).
        showAlarmToast('🔗','Lien enregistré', 'Analyse automatique impossible depuis ce navigateur (restriction de sécurité du site distant) — complétez les champs ci-dessous, l\'adresse reste bien liée.', 'warning', 6000);
        finalize({title: fallbackTitle, description:'', steps:[]});
    });
}
...
function analyzeProcedureImport(){
    ...
    safeText('protocol-modal-title', 'Nouvelle Procédure (importée)');
    safeStyle('protocol-import-panel','display', 'none');
    safeStyle('protocol-edit-panel','display', 'block');
    populateProtocolEditForm(activeProtocolDraft);
    renderProtocolModalFooter('edit');
