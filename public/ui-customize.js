...
function openUiCustomizeModal(){
    ...
    document.getElementById('uiCustomizeModal').classList.add('active');
...
function tileActionHandlerString(tile){
    ...
    if(a.type === 'url')  return `window.open('${(a.target||'').replace(/'/g,"\\'")}','_blank')`;
    return '';
}
...
function toggleHomeEditMode(){
    ...
    document.getElementById('home-edit-toggle').classList.toggle('active', homeEditMode);
    renderHomeTiles();
}
...
function openTileEditor(id){
    ...
    document.getElementById('tile-editor-title').textContent = id ? 'Modifier la tuile' : 'Nouvelle tuile';
    ...
    document.getElementById('tile-edit-delete-btn').style.display = (id && !isCore) ? '' : 'none';
    document.getElementById('tile-edit-duplicate-btn').style.display = id ? '' : 'none';

    document.getElementById('tileEditorModal').classList.add('active');
}
function onTileActionTypeChange(){
    const type = document.getElementById('tile-edit-action-type').value;
    document.getElementById('tile-edit-target-bound-wrap').style.display = type==='bound' ? '' : 'none';
    document.getElementById('tile-edit-target-view-wrap').style.display  = type==='view'  ? '' : 'none';
    document.getElementById('tile-edit-target-url-wrap').style.display   = type==='url'   ? '' : 'none';
}
...
function openIconCropperFor(targetKey){
    ...
    document.getElementById('uic-cropper-wrap').style.display = 'none';
    document.getElementById('uic-cropper-zoom-row').style.display = 'none';
    ...
    document.getElementById('iconCropperModal').classList.add('active');
}
...
function loadImageIntoCropper(dataUrl){
    ...
    img.onload = function(){
        ...
        document.getElementById('uic-cropper-zoom-row').style.display = 'flex';
        document.getElementById('uic-cropper-zoom').value = 1;
        document.getElementById('uic-cropper-zoom-val').textContent = '100%';
        fitCropperImage();
...
function onIconCropperZoom(val){
    ...
    document.getElementById('uic-cropper-zoom-val').textContent = Math.round(iconCropperState.zoom*100) + '%';
    updateCropperTransform();
}
function attachCropperDragHandlers(){
    const wrap = document.getElementById('uic-cropper-wrap');
    if(wrap.dataset.dragBound) return; // évite les doublons d'écouteurs entre ouvertures
...
function confirmIconCropper(){
    ...
    const wrapSize = document.getElementById('uic-cropper-wrap').clientWidth || 260;
