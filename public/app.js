...
function ruleDescription(rule){
    ...
    if(rule.recType==='biweekly')  return 'Tous les 15 jours';
    if(rule.recType==='monthlyX')  return `Tous les ${rule.interval||1} mois`;
    if(rule.recType==='yearlyX')   return `Tous les ${rule.interval||1} ans`;
...
function openTaskModal(taskId){
    ...
    document.getElementById('task-date').value       = activeDate;
    document.getElementById('task-date').disabled    = !!task.recRuleId; // occurrence récurrente : date non déplaçable
...
function saveTask(){
    ...
    if(activeTaskId){
        ...
        if(task){
            ...
            // Déplacement vers une autre date (impossible pour une occurrence
            // récurrente : le champ date est désactivé dans ce cas côté formulaire)
            if(!task.recRuleId && dateVal !== activeDate){
        ...
        if(recType !== 'none'){
            ...
            const rule = {id:generateUUID(), startDate:dateVal, recType, selectedDays, interval, ...data};
