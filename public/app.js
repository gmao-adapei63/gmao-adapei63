...
const DEFAULT_NAV = [
    ...
    {id:'protocols', icon:'🚨', label:'Protocoles',  type:'view', target:'protocols'},
    {id:'plans',     icon:'🗺️', label:'Plans',       type:'view', target:'plans'},
];
...
function ensureStateDefaults(){
    ...
    // Ajoute automatiquement les nouveaux boutons (protocoles/plans) aux navConfig existants
    ['protocols','plans'].forEach(id=>{
        if(!appState.navConfig.find(b=>b.id===id)){
            const def = DEFAULT_NAV.find(b=>b.id===id);
...
const PROTOCOL_CATEGORIES = [
    {id:'incendie',  icon:'🔥', label:'Incendie'},
    {id:'elec',      icon:'⚡', label:'Panne électrique'},
    {id:'eau',       icon:'💧', label:'Panne eau / plomberie'},
    {id:'gaz',       icon:'🧯', label:'Fuite / panne gaz'},
    {id:'degradee',  icon:'⚠️', label:'Marche dégradée'},
    {id:'autre',     icon:'📋', label:'Autre procédure'},
];
...
function renderProtocols(){
    ...
    container.innerHTML = list.map(p=>{
        const cat = PROTOCOL_CATEGORIES.find(c=>c.id===p.category) || PROTOCOL_CATEGORIES[5];
        return `
        <div class="info-list-item" onclick="openProtocolModal('${p.id}')">
            <div class="info-list-dot ${p.category==='incendie'||p.category==='gaz'?'danger':(p.category==='degradee'?'warning':'info')}"></div>
            <div class="info-list-text">
                <div class="info-list-title">${cat.icon} ${p.title}</div>
                <div class="info-list-sub">${cat.label}${p.updatedAt ? ' · maj '+new Date(p.updatedAt).toLocaleDateString('fr-FR') : ''}</div>
            </div>
        </div>`;
    }).join('');
}
...
function openCreateProtocolModal(){
    ...
    document.getElementById('protocol-category').value = 'incendie';
    document.getElementById('protocol-content').value = '';
    document.getElementById('btn-delete-protocol').style.display = 'none';
...
function openProtocolModal(id){
    ...
    if(!p) return;
    document.getElementById('protocol-modal-title').innerText = p.title;
    document.getElementById('protocol-title').value = p.title;
    document.getElementById('protocol-category').value = p.category;
    document.getElementById('protocol-content').value = p.content||'';
    document.getElementById('btn-delete-protocol').style.display = 'inline-flex';
    document.getElementById('protocolModal').classList.add('active');
}
function saveProtocol(){
    ...
    const content  = document.getElementById('protocol-content').value;
    if(!appState.protocols) appState.protocols = [];
    if(activeProtocolId){
        const p = appState.protocols.find(x=>x.id===activeProtocolId);
        if(p) Object.assign(p, {title, category, content, updatedAt:new Date().toISOString()});
    } else {
        appState.protocols.push({id:generateUUID(), title, category, content, updatedAt:new Date().toISOString()});
    }
    saveData();
    renderProtocols();
