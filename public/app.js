...
const HOME_ELEMENTS = [
    ...
    {id:'actions-fab',    icon:'🎯', label:'Bouton flottant Actions'},
    {id:'suivis-fab',     icon:'📋', label:'Bouton flottant Suivis en cours'},
];
...
function ensureStateDefaults(){
    ...
    // Moteur de suivis génériques (V2) — additif, voir public/suivis/
    if(window.SuivisEngine) SuivisEngine.ensureDefaults();
...
// ─────────────────────────────────────────────────────────────────
// PERSISTANCE — local-first + Firestore temps réel partagé.
//
// 1) LOCAL D'ABORD : chaque modification est écrite IMMÉDIATEMENT dans
//    localStorage (cache complet des données GMAO), sur CE terminal, avant
//    même de tenter le réseau. Une coupure réseau ou une erreur Firebase ne
//    peut donc jamais faire perdre une saisie : elle reste sur l'appareil et
//    repart automatiquement dès que la connexion revient (voir plus bas).
// 2) PARTAGE TEMPS RÉEL : dès que le réseau est là, la donnée part vers
//    Firestore et se propage instantanément à tous les autres appareils via
//    l'écouteur temps réel (onSnapshot) — sans F5, sans bouton manuel.
// 3) GARDE ANTI-ÉCRASEMENT (héritée de la correction précédente, conservée
//    à l'identique) : tant qu'aucune confirmation réelle du serveur n'a été
//    reçue (_serverSyncReady === false), AUCUNE écriture ne part vers
//    Firestore. Le cache local ci-dessous ne contourne jamais cette règle :
//    il sert uniquement à (a) afficher les dernières données connues sans
//    attendre le réseau et (b) mémoriser qu'une sauvegarde reste à envoyer.
//    La comparaison d'horodatage (_savedAt) reste l'arbitre final en cas de
//    conflit entre ce que contient le cache local et ce que renvoie le
//    serveur — la donnée la plus récente gagne toujours.
// 4) PRÉFÉRENCES D'AFFICHAGE (thème, densité, disposition des tuiles,
//    position du bouton flottant, caméra QR, config GitHub locale…) restent
//    dans une clé localStorage séparée, propre à cet appareil, et ne sont
//    JAMAIS synchronisées vers Firestore. Voir ui-customize.js.
// ─────────────────────────────────────────────────────────────────
const SERVER_BACKUP_URL = '/api/gmao/backup'; // ancien secours Replit — inutilisé sur GitHub Pages, conservé sans effet de bord
const LOCAL_DATA_CACHE_KEY = 'gmao_local_data_cache'; // cache local des DONNÉES (distinct des préférences d'affichage)
...
// Écrit immédiatement l'état courant dans le cache local du terminal.
// pendingSync=true tant que cette version n'a pas été confirmée envoyée à Firestore.
function saveLocalCache(pendingSync){
    try {
        localStorage.setItem(LOCAL_DATA_CACHE_KEY, JSON.stringify({
            data: appState,
            savedAt: appState._savedAt,
            pendingSync: !!pendingSync
        }));
    } catch(e){
        // Quota localStorage dépassé ou navigation privée : on log, mais on ne bloque jamais
        // la sauvegarde (Firestore reste tenté normalement).
        console.warn('Cache local indisponible :', e.message);
    }
}
function loadLocalCache(){
    try {
        const raw = localStorage.getItem(LOCAL_DATA_CACHE_KEY);
        return raw ? JSON.parse(raw) : null;
    } catch(e){
        console.warn('Lecture du cache local impossible :', e.message);
        return null;
    }
}
function localCacheHasPendingSync(){
    const c = loadLocalCache();
    return !!(c && c.pendingSync);
}

// Re-rend tout ce qui dépend des données après une mise à jour reçue (locale ou distante),
// pour que chaque module (missions, agents, équipements, véhicules, protocoles, actions,
// plans) reste à jour sans navigation manuelle ni F5.
function refreshAllViewsAfterDataUpdate(){
    renderBottomNav();
    loadDayData(document.getElementById('system-date').value);
    renderAgents(); renderEquipment(); renderVehicles();
    if(typeof renderProtocolTabs === 'function') renderProtocolTabs();
...
function markServerSyncReady(){
    ...
    if(_pendingSaveWhileSyncing || localCacheHasPendingSync()){
        _pendingSaveWhileSyncing = false;
        pushToFirebase(true); // rattrape la sauvegarde qui avait été mise en attente (y compris entre deux sessions)
    }
}

function loadData(){
    // 1) Affichage immédiat depuis le cache local du terminal (aucune attente réseau,
    //    plus d'écran vide le temps que Firestore réponde).
    ...
    // 2) Confirmation/rattrapage depuis Firestore. L'écouteur temps réel
...
function saveData(){
    ...
    saveLocalCache(true); // écrit en local IMMÉDIATEMENT, avant toute tentative réseau
    updateDashboard();
    if(window.SearchEngine) SearchEngine.rebuild();
    scheduleGitHubPush();
    pushToFirebase(true); // tentative d'écriture immédiate vers Firestore — la synchronisation entre appareils doit être instantanée, pas différée
}
...
// Retour réseau : on ne se contente plus d'attendre le prochain intervalle ou la
// prochaine saisie — on relance immédiatement une synchronisation dans les deux sens
// (récupération de ce qui a changé ailleurs pendant la coupure, puis envoi de toute
// sauvegarde locale restée en attente).
window.addEventListener('online', () => {
    console.log('🌐 Réseau rétabli — synchronisation immédiate');
    pullFromFirebase(true).then(()=>{
        if(_pendingSaveWhileSyncing || localCacheHasPendingSync()) pushToFirebase(true);
    });
});
...
function navItemClick(id){
    ...
    if(btn.type==='view'){
        ...
        // Ouvrir URL / lien appli — intégré à l'appli ou nouvel onglet selon préférence
        openExternalLink(btn.target, btn.label);
        highlightNav(id);
    }
}

// ─────────────────────────────────────────────────────────────────
// OUVERTURE INTÉGRÉE DES LIENS / APPLIS EXTERNES
// (boutons bas, tuiles d'accueil…) — préférence par appareil (localStorage)
// ─────────────────────────────────────────────────────────────────
function getLinkOpenMode(){
    try{ return localStorage.getItem('gmao_link_mode') || 'inline'; }
    catch(e){ return 'inline'; }
}
function setLinkOpenMode(mode){
    try{ localStorage.setItem('gmao_link_mode', mode==='newtab' ? 'newtab' : 'inline'); }catch(e){}
}
function isNativeLink(url){
    return /^(intent:|market:|tel:|mailto:|sms:|geo:|whatsapp:)/i.test(url||'');
}
// Liens d'authentification (Keycloak/OAuth-OIDC générique + ageval.fr en secours) :
// ces serveurs renvoient quasi systématiquement X-Frame-Options / CSP frame-ancestors
// qui interdisent l'iframe (ERR_BLOCKED_BY_RESPONSE) → toujours ouvrir nativement.
function isAuthLink(url){
    return /\/auth\/realms\/|\/protocol\/openid-connect\/|ageval\.fr/i.test(url||'');
}
function openExternalLink(url, label){
    if(!url) return;
    if(isNativeLink(url) || isAuthLink(url) || getLinkOpenMode()==='newtab'){ window.open(url, '_blank'); return; }
    openWebview(url, label);
}
function openWebview(url, label){
    ...
    const nav    = document.getElementById('bottom-nav');
    overlay.style.top    = (header ? header.offsetHeight : 64) + 'px';
    overlay.style.bottom = (nav ? nav.offsetHeight : 80) + 'px';
    frame.src = url;
    safeText('webview-title', label || '');
...
function openWebviewExternally(){
    const frame = document.getElementById('webview-frame');
    if(frame && frame.src && frame.src !== 'about:blank') window.open(frame.src, '_blank');
}

// ─────────────────────────────────────────────────────────────────
// BOUTON CONSOLE DE DÉBOGAGE (ERUDA) — activable/désactivable
// ─────────────────────────────────────────────────────────────────
function setConsoleEnabled(enabled){
    try{ localStorage.setItem('gmao_console_enabled', enabled ? 'true' : 'false'); }catch(e){}
    if(enabled){
        if(!window.eruda){
...
function toggleSidebar(){ closeWebview(); safeToggleClass('.sidebar','active'); safeToggleClass('.sidebar-overlay','active'); }
...
function switchView(viewName, _isBack){
    // Un bouton de navigation (bas ou header) doit toujours reprendre la main
    // sur la surcouche webview intégrée, sans passer par le ✖ de la mini-barre.
    closeWebview();
    ...
    if(viewName==='report'){ initReportPeriodDefaults(); renderReportPreview(); }
    if(viewName==='protocols') renderProtocols();
    if(viewName==='plans')     renderPlans();
    if(viewName==='suivis' && window.SuivisView) SuivisView.renderAccueil();
}
...
// ─────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────
// RAPPORT — sélection de période à afficher/imprimer
// (jour / semaine ISO / mois / année / plage personnalisée, en plus
//  du mode par défaut "14 derniers jours" préexistant)
// ─────────────────────────────────────────────────────────────────
function reportPad2(n){ return String(n).padStart(2,'0'); }
function reportDateStr(d){ return `${d.getFullYear()}-${reportPad2(d.getMonth()+1)}-${reportPad2(d.getDate())}`; }
function initReportPeriodDefaults(){
    const today = reportDateStr(new Date());
    const single = document.getElementById('report-date-single');
...
function onReportPeriodChange(){
    const mode = (document.getElementById('report-period-mode')||{}).value || 'recent';
    const singleWrap = document.getElementById('report-date-single-wrap');
    const rangeWrap   = document.getElementById('report-date-range-wrap');
    if(singleWrap) singleWrap.style.display = (mode!=='recent' && mode!=='personnalise') ? 'flex' : 'none';
    if(rangeWrap)  rangeWrap.style.display  = (mode==='personnalise') ? 'flex' : 'none';
    renderReportPreview();
}
// Retourne [dateDébut, dateFin] (chaînes AAAA-MM-JJ) pour un mode et une date d'ancrage donnés
function computeReportRange(mode, anchorStr){
    const anchor = anchorStr ? new Date(anchorStr+'T00:00:00') : new Date();
    if(mode==='jour'){
        const s = reportDateStr(anchor);
        return [s, s];
    }
    if(mode==='semaine'){
        const offset = (anchor.getDay()+6)%7; // 0=lundi … 6=dimanche
        const monday = new Date(anchor); monday.setDate(anchor.getDate()-offset);
        const sunday = new Date(monday); sunday.setDate(monday.getDate()+6);
        return [reportDateStr(monday), reportDateStr(sunday)];
    }
    if(mode==='mois'){
        const first = new Date(anchor.getFullYear(), anchor.getMonth(), 1);
        const last  = new Date(anchor.getFullYear(), anchor.getMonth()+1, 0);
        return [reportDateStr(first), reportDateStr(last)];
    }
    if(mode==='annee'){
...
function reportPeriodLabel(mode, from, to){
    const fmt = s=>{ const [y,m,d]=s.split('-'); return `${d}/${m}/${y}`; };
    if(mode==='recent')       return '14 derniers jours';
    if(mode==='jour')         return `Jour du ${fmt(from)}`;
    if(mode==='semaine')      return `Semaine du ${fmt(from)} au ${fmt(to)}`;
    if(mode==='mois')         return `Mois du ${fmt(from)} au ${fmt(to)}`;
    if(mode==='annee')        return `Année ${from.split('-')[0]}`;
    if(mode==='personnalise') return `Du ${fmt(from)} au ${fmt(to)}`;
    return '';
}
function renderReportPreview(){
    const modeEl = document.getElementById('report-period-mode');
    const mode = modeEl ? modeEl.value : 'recent';
    let dates, from, to;
    if(mode==='recent'){
        dates = Object.keys(appState.calendar).sort().reverse().slice(0,14);
        from = dates.length ? dates[dates.length-1] : '';
        to   = dates.length ? dates[0] : '';
    } else if(mode==='personnalise'){
        from = (document.getElementById('report-date-start')||{}).value || '0000-00-00';
        to   = (document.getElementById('report-date-end')||{}).value   || '9999-99-99';
        dates = Object.keys(appState.calendar).filter(d=> d>=from && d<=to).sort();
    } else {
        const anchorStr = (document.getElementById('report-date-single')||{}).value;
        [from, to] = computeReportRange(mode, anchorStr);
        dates = Object.keys(appState.calendar).filter(d=> d>=from && d<=to).sort();
    }
    safeText('report-period-label', dates.length ? reportPeriodLabel(mode, from, to) : '');
    if(!dates.length){ safeHTML('report-preview', '<p>Aucune donnée pour cette période.</p>'); return; }
...
function openSettingsModal(){
    ...
    // Préférences locales à cet appareil
    const linkModeEl = document.getElementById('settings-link-mode');
    if(linkModeEl) linkModeEl.value = getLinkOpenMode();
    const consoleEl = document.getElementById('settings-console-enabled');
    if(consoleEl){
        let consoleOn = false;
        try{ consoleOn = localStorage.getItem('gmao_console_enabled') === 'true'; }catch(e){}
        consoleEl.checked = consoleOn;
    }
...
// ─────────────────────────────────────────────────────────────────
// SYNC FIREBASE FIRESTORE — multi-appareils, instantané, partagé en temps
// réel (voir onSnapshot plus bas). Le cache local (localStorage) sert de
// filet de sécurité hors-ligne et de file d'attente persistante — voir
// saveLocalCache()/loadLocalCache() plus haut. Firestore reste la source de
// vérité partagée dès que le réseau est disponible.
// ─────────────────────────────────────────────────────────────────
...
async function pushToFirebase(silent=false){
    ...
    try {
        ...
        appState._savedAt = payload._savedAt;
        saveLocalCache(false); // confirmé envoyé : le cache local n'est plus "en attente"
        saveVersionSnapshot(payload); // historique des 5 dernières versions (voir plus bas)
...
async function pullFromFirebase(silent=false){
    ...
    try {
        ...
        if(remoteTs > localTs){
            ...
            ensureStateDefaults();
            saveLocalCache(false); // donnée serveur confirmée = pas en attente d'envoi
            refreshAllViewsAfterDataUpdate();
...
// ─────────────────────────────────────────────────────────────────
// HISTORIQUE DES VERSIONS — conserve les 5 dernières versions synchronisées,
// pour pouvoir revenir en arrière en cas d'erreur (suppression accidentelle,
// mauvaise modification…). Stocké dans une sous-collection Firestore
// distincte du document principal, pour ne jamais alourdir ce dernier.
// N'affecte jamais la garde anti-écrasement (_serverSyncReady) : c'est un
// journal en plus, pas un chemin d'écriture supplémentaire vers le document.
// ─────────────────────────────────────────────────────────────────
const FS_HISTORY_LIMIT = 5;

function getFsHistoryCollectionRef(){
    if(!window._firebaseDb || !window._fsCollection) return null;
    return window._fsCollection(window._firebaseDb, 'gmao', FS_DOC_ID, 'history');
}

// Appelée après chaque écriture réussie du document principal. Volontairement
// non bloquante : un échec ici ne doit jamais empêcher ni ralentir la sauvegarde
// des données elles-mêmes.
async function saveVersionSnapshot(payload){
    try {
        const col = getFsHistoryCollectionRef();
        if(!col || !window._fsAddDoc) return;
        await window._fsAddDoc(col, { data: payload, savedAt: payload._savedAt || new Date().toISOString() });
        await trimVersionHistory();
    } catch(e){
        console.warn('Historique des versions : écriture ignorée -', e.message);
    }
}

// Ne garde que les FS_HISTORY_LIMIT versions les plus récentes.
async function trimVersionHistory(){
    try {
        const col = getFsHistoryCollectionRef();
        if(!col || !window._fsQuery || !window._fsGetDocs) return;
        const q = window._fsQuery(col, window._fsOrderBy('savedAt','desc'));
        const snap = await window._fsGetDocs(q);
        const docs = snap.docs;
        if(docs.length <= FS_HISTORY_LIMIT) return;
        const toDelete = docs.slice(FS_HISTORY_LIMIT);
        for(const d of toDelete){
            await window._fsDeleteDoc(d.ref);
        }
    } catch(e){
        console.warn('Historique des versions : nettoyage ignoré -', e.message);
    }
}

// Récupère les versions disponibles (les plus récentes en premier) pour affichage.
async function listVersionHistory(){
    try {
        const col = getFsHistoryCollectionRef();
        if(!col || !window._fsQuery || !window._fsGetDocs) return [];
        const q = window._fsQuery(col, window._fsOrderBy('savedAt','desc'), window._fsLimit(FS_HISTORY_LIMIT));
        const snap = await window._fsGetDocs(q);
        return snap.docs.map(d => ({ id: d.id, savedAt: d.data().savedAt, data: d.data().data }));
    } catch(e){
        console.warn('Historique des versions : lecture impossible -', e.message);
        return [];
    }
}

// Restaure une version antérieure comme état courant (avec confirmation appelante côté UI).
// La version restaurée devient une nouvelle sauvegarde à part entière : elle repasse par
// saveData()/pushToFirebase() normalement, et génère donc elle-même une nouvelle entrée
// d'historique — rien n'est perdu, on peut toujours revenir en arrière une seconde fois.
function restoreVersionSnapshot(versionData){
    appState = Object.assign({
        recurringRules:[], deletedInstances:{},
        navConfig: JSON.parse(JSON.stringify(DEFAULT_NAV))
    }, versionData);
