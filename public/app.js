...
let _qrLoop      = null;   // setInterval handle
let _qrDetector  = null;   // BarcodeDetector instance (si dispo)
...
async function openScanner(){
    _qrDone = false;
    _qrTorchOn = false;
    document.getElementById('qrModal').classList.remove('qr-detected');
    ...
    // Détecteur natif rapide si dispo, sinon jsQR (JS pur, universel et fiable :
    // fonctionne sur Safari/Firefox/anciens Android là où BarcodeDetector est absent).
    ...
    try {
        ...
        // ── Capacités caméra : zoom natif + flash (torche) ──────────────
        _qrTrack = _qrStream.getVideoTracks()[0] || null;
        setupQrCameraControls(_qrTrack);

        if(hasBD){
            _qrDetector = new BarcodeDetector({formats:['qr_code','data_matrix','code_128','code_39','ean_13','ean_8','upc_a','upc_e']});
        }

        let frameCount = 0;
        // Boucle d'affichage ~24 fps : rendu fluide + tentative de détection à
        // (presque) chaque frame pour une reconnaissance rapide et fiable.
        _qrLoop = setInterval(async () => {
            if(_qrDone || !_qrStream) return;
            if(video.readyState < 2) return;

            // 1. Dessiner la frame vidéo en COULEUR PLEINE sur le canvas
            //    (évite le bug YUV vert de certaines WebViews Android) — aucun
            //    filtre n'est appliqué : c'est le flux caméra natif, non altéré.
            ctx.drawImage(video, 0, 0, W, H);

            frameCount++;
            let detected = null;

            // 2. Détecteur natif (rapide) si disponible
            ...
            // 3. Repli jsQR (JS pur) toutes les 2 frames — universel et robuste,
            //    y compris sur les navigateurs sans BarcodeDetector.
            if(!detected && hasJsQR && frameCount % 2 === 0){
                try {
                    const img = ctx.getImageData(0, 0, W, H);
                    const res = jsQR(img.data, W, H, { inversionAttempts: 'attemptBoth' });
                    if(res && res.data) detected = res.data;
                } catch(_){}
            }
...
function setupQrCameraControls(track){
    ...
    if(!track || !track.getCapabilities){ zoomBar.style.display = 'none'; return; }
    let caps = {};
    try { caps = track.getCapabilities() || {}; } catch(_){ caps = {}; }

    zoomBar.style.display = 'flex'; // le repli CSS fonctionne toujours, même sans capacité native
...
function toggleQrTorch(){
    ...
    document.getElementById('qr-torch-btn').classList.toggle('active', _qrTorchOn);
}
...
function handleQrResult(text){
    ...
    // QR Code reconnu : retour visuel + navigation immédiate, sans étape intermédiaire
    document.getElementById('qrModal').classList.add('qr-detected');
...
function resolveQrTarget(raw){
    ...
    // Module Actions (ACT-xxxxxx) — priorité car identifiant explicitement préfixé
    if(/^ACT-\d+$/i.test(raw)){
        const act = (appState.actions||[]).find(a=>a.id.toLowerCase()===lower);
        if(act) return { open(){ openActionsModule(); openActionEditor(act.id); } };
        const trashed = (appState.actionsTrash||[]).find(a=>a.id.toLowerCase()===lower);
        if(trashed) return { open(){ openActionsModule(); openActionsTrash(); } };
        return null; // identifiant Action reconnu dans sa forme mais introuvable
    }
    ...
    // Action (recherche par ID sans le préfixe explicite, ou par titre)
    const act2 = (appState.actions||[]).find(a => a.id===raw || _qrNormalize(a.title)===rn);
    if(act2) return { open(){ openActionsModule(); openActionEditor(act2.id); } };
...
async function closeScanner(){
    ...
    _qrTorchOn = false; _qrTrack = null;
    ...
    document.getElementById('qr-torch-btn').classList.remove('active');
    document.getElementById('qrModal').classList.remove('qr-detected');
    closeModal('qrModal');
}
