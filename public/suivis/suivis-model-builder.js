...
const SuivisModelBuilder = (function(){
        ...
        const html = `<div class="suivis-header">
            ...
            <div class="suivis-builder-section">
                ...
                <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:4px">
                    <button type="button" class="btn-secondary" onclick="document.getElementById('suivis-exec-file').click()">📥 Choisir un fichier .html</button>
                    <input type="file" id="suivis-exec-file" accept=".html,.htm,text/html" style="display:none" onchange="SuivisModelBuilder.majExecFile(this.files[0])">
                    <span id="suivis-exec-file-name" style="font-size:.8rem;color:var(--text-muted)">${t.execFile ? '📄 '+esc(t.execFile.name)+' (importé)' : ''}</span>
                    ${t.execFile ? `<button type="button" class="btn-icon" onclick="SuivisModelBuilder.retirerExecFile()" title="Retirer">🗑️</button>` : ''}
                </div>
                <p style="font-size:.72rem;color:var(--text-muted);margin:2px 0 0">
                    Si l'une des deux options est renseignée, cliquer sur une campagne de ce suivi ouvrira directement cette page au lieu de la fiche interne (qui reste accessible via l'icône 🗄️).
                </p>
            </div>
            <div class="suivis-builder-section">
    ...
    function majExecFile(file){
        if(!file) return;
        const reader = new FileReader();
        reader.onload = () => {
            travail.execFile = {name:file.name, html:reader.result};
            renderApercu();
    ...
    function retirerExecFile(){ travail.execFile = null; renderApercu(); }
    ...
    return {
        ...
        majMeta, majExecFile, retirerExecFile, majChamp, retirerChamp, ajouterChamp,
