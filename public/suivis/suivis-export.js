...
const SuivisExport = (function(){
    ...
    function exporterPDF(campagneId){
        ...
        const doc = new jsPDF({orientation: (champsDef.length+nbColsControles) > 7 ? 'landscape' : 'portrait', unit:'pt'});
        ...
        const head = [
            [
                ...champsDef.map(cf => ({content:cf.label, rowSpan:2})),
        ...
        const body = [];
        sections.forEach(section => {
            body.push([{content:section.nom, colSpan:champsDef.length+nbColsControles,
                styles:{fillColor:rgbClair, textColor:[20,20,20], fontStyle:'bold'}}]);
            SuivisEngine.listItemsForSection(section.id).forEach(item => {
                const cellulesChamps = champsDef.map(cf => item.champs[cf.id]||'');
                const cellulesControles = controlesDef.flatMap(c => {
                    const val = item.controles[c.id] || {};
                    return [val.etat||'', val.date||''];
                });
                body.push([...cellulesChamps, ...cellulesControles]);
            });
        });
