...
const SuivisView = (function(){
                ...
                if(campagnes.length === 0){
                    ...
                    campagnes.forEach(c => {
                        ...
                        html += `<div class="suivis-campagne-row" onclick="SuivisView.pratiquerCampagne('${c.id}')">
                            <div class="suivis-campagne-nom">${m.execUrl ? '🛂 ' : ''}${escapeHtml(SuivisEngine.titreCampagne(c.id))}</div>
                            ...
                            ${m.execUrl ? `<button class="btn-icon" style="margin-top:4px" onclick="event.stopPropagation();SuivisView.ouvrirCampagne('${c.id}')" title="Voir la fiche interne (base de données)">🗄️ Fiche interne</button>` : ''}
                        </div>`;
                    });
                    html += `<button class="btn-secondary" onclick="SuivisView.nouvelleCampagne('${m.id}')">+ Nouvelle campagne</button>`;
                }
    ...
    // ── Pratiquer une campagne sur le terrain : si le modèle a une page
    // liée (execUrl), on l'ouvre directement (webview intégrée) — la fiche
    // interne (base de données) reste accessible via l'icône 🗄️. Sinon,
    // comportement inchangé : on ouvre la fiche interne classique.
    ...
    function ouvrirCampagne(campagneId){
        ...
        let html = `<div class="suivis-header">
            ...
            ${modele.execUrl ? `<button class="btn-icon" onclick="SuivisView.pratiquerCampagne('${campagneId}')" title="Ouvrir la page de pratique terrain">🛂</button>` : ''}
        ...
        if(!campagnes.length){
            ...
            campagnes.forEach(c => {
                ...
                html += `<div class="suivis-campagne-row" style="border-left:4px solid ${modele?modele.couleur:'#3b82f6'}" onclick="SuivisView.pratiquerCampagne('${c.id}')">
                    <div class="suivis-campagne-nom">${modele && modele.execUrl ? '🛂 ' : (modele?modele.icone:'📋')+' '} ${escapeHtml(modele?modele.nom:'')} — ${escapeHtml(SuivisEngine.titreCampagne(c.id))}</div>
                    <div class="suivis-progress-bar"><div class="suivis-progress-fill" style="width:${pct}%"></div></div>
                    <div class="suivis-campagne-stats">${prog.fait}/${prog.total} · ${pct}% ${anom ? '· <span style="color:var(--danger)">'+anom+' anomalie(s)</span>' : ''}</div>
                    ${modele && modele.execUrl ? `<button class="btn-icon" style="margin-top:4px" onclick="event.stopPropagation();SuivisView.ouvrirCampagne('${c.id}')" title="Voir la fiche interne (base de données)">🗄️ Fiche interne</button>` : ''}
                </div>`;
            });
            html += '</div>';
        }
        ...
        const html = `<div class="suivis-header">
            ...
            <div class="suivis-builder-section">
                ...
                <p style="font-size:.72rem;color:var(--text-muted)">
                    Si renseignée, cliquer sur une campagne de ce suivi ouvrira directement cette page (la fiche interne reste accessible via 🗄️).
                </p>
    ...
    function enregistrerApparenceModele(modeleId){
        ...
        SuivisEngine.updateModele(modeleId, {icone, couleur, execUrl});
