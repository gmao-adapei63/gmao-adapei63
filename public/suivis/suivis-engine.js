...
const SuivisEngine = (function(){
    ...
    function ensureDefaults(){
        ...
        // updateTileSuivis() a besoin du DOM (tile-suivis) : différé au tick suivant
        // pour ne jamais dépendre de l'ordre de chargement des scripts.
        setTimeout(updateTileSuivis, 0);
    }
    ...
    function registerSearchProviderOnce(){
        ...
        window.registerSearchProvider('suivis', function(){
            ...
            return (s.items||[]).map(it=>{
                ...
                const cp = modele && (modele.champsItem||[])[0];
                const cs = modele && (modele.champsItem||[])[1];
                return {
                    type: 'suivi_item',
                    icon: (modele && modele.icone) || '🗂️',
                    category: (modele && modele.nom) || 'Suivi',
                    title: [cp && it.champs[cp.id], cs && it.champs[cs.id]].filter(Boolean).join(' — ') || 'Item',
    ...
    function createModele(def){
        const modele = Object.assign({
            ...
            groupePar: 'section',
            source: null,
            execUrl: '',   // page HTML liée (ex. OneDrive) pour la pratique terrain de ce suivi
            execFile: null, // {name, html} — page HTML importée directement (contourne les blocages OneDrive/SharePoint)
    ...
    // Résumé utilisé par la double confirmation avant suppression d'un modèle
    // entier (voir SuivisView.demanderSuppressionModele). Additionne toutes
    // les campagnes de ce modèle.
    function resumeModelePourSuppression(modeleId){
        const modele = getModele(modeleId);
        if(!modele) return null;
        const campagnes = listCampagnesForModele(modeleId);
        let nbItems = 0, controlesSaisis = 0, photos = 0, signatures = 0;
        campagnes.forEach(c => {
            const r = resumeCampagnePourSuppression(c.id);
            if(!r) return;
            nbItems += r.nbItems; controlesSaisis += r.controlesSaisis;
            photos += r.photos; signatures += r.signatures;
        });
        return { nom: modele.nom, nbCampagnes: campagnes.length, nbItems, controlesSaisis, photos, signatures };
    }

    // Suppression en cascade complète : le modèle + toutes ses campagnes
    // (donc leurs sections/items/pièces jointes, via deleteCampagne).
    // Appelée seulement après la double confirmation côté vue.
    function deleteModele(modeleId){
        const s = appState.suivis;
        listCampagnesForModele(modeleId).forEach(c => deleteCampagne(c.id));
    ...
    // Clôture de fin de campagne : demandée une seule fois (contrôleur,
    // qualification, date, commentaires généraux, signature). Ne verrouille
    // pas encore la ressaisie des contrôles (prévu avec le code admin,
    // pas encore implémenté) — pour l'instant marque juste la campagne
    // "terminee" et fige ces informations pour l'impression/export.
    function cloturerCampagne(campagneId, valeurs){
        const campagne = getCampagne(campagneId);
        if(!campagne) return {ok:false, error:'Campagne introuvable'};
        campagne.statut = 'terminee';
        campagne.controleur = (valeurs && valeurs.controleur) || '';
        campagne.qualification = (valeurs && valeurs.qualification) || '';
        campagne.dateControle = (valeurs && valeurs.dateControle) || new Date().toISOString().split('T')[0];
        campagne.commentaireGeneral = (valeurs && valeurs.commentaireGeneral) || '';
        if(valeurs && valeurs.signature) campagne.signature = valeurs.signature;
        campagne.clotureAt = new Date().toISOString();
        persist();
        return {ok:true};
    }

    function reouvrirCampagne(campagneId){
        const campagne = getCampagne(campagneId);
        if(!campagne) return {ok:false, error:'Campagne introuvable'};
    ...
    // ─────────────────────────────────────────────────────────────
    // CAMPAGNES INACHEVÉES (toutes catégories) — tuile d'accueil
    // ─────────────────────────────────────────────────────────────
    function listCampagnesInachevees(){
        return (appState.suivis.campagnes || []).filter(c => c.statut !== 'terminee');
    }

    function updateTileSuivis(){
        const tile = document.getElementById('tile-suivis');
        if(tile) tile.textContent = String(listCampagnesInachevees().length);
        if(typeof window.updateSuivisFabBadge === 'function') window.updateSuivisFabBadge();
    }

    // ─────────────────────────────────────────────────────────────
    // NAVIGATION SÉQUENTIELLE — flux de contrôle rapide OK/HS
    // ─────────────────────────────────────────────────────────────
    // Premier type de contrôle non renseigné sur UN item donné, dans
    // l'ordre défini par le modèle (ex. "visuel" avant "décharge").
    function premierControleNonFait(itemId){
        const item = getItem(itemId);
        if(!item) return null;
        const modele = getModele(item.modeleId);
        for(const ctrl of (modele.controles||[])){
            const val = item.controles[ctrl.id] || {};
            if(!val.etat) return ctrl.id;
        }
        return null;
    }

    // Prochain équipement (et type de contrôle) à traiter après apresItemId,
    // dans l'ordre section → item → type de contrôle. Sans apresItemId,
    // reprend depuis le tout début de la campagne. Retourne null si tout
    // est renseigné (fin de campagne).
    function prochainPosteAControler(campagneId, apresItemId){
        const sections = listSectionsForCampagne(campagneId);
        let franchiApres = !apresItemId;
        for(const section of sections){
            for(const item of listItemsForSection(section.id)){
                if(franchiApres){
                    const ctrlId = premierControleNonFait(item.id);
                    if(ctrlId) return {itemId:item.id, ctrlId};
                }
                if(item.id === apresItemId) franchiApres = true;
            }
        }
        return null;
    }
